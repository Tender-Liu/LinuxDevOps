...
## 教学目标
1. 理解数据库是什么 ：通过生活中的例子，明白数据库是做什么的，以及它和 Excel 的区别。
2. 了解数据库的分类 ：知道数据库主要分为关系型（如 MySQL）和非关系型（如 Redis），并了解它们各自的特点。
3. 认识主流数据库产品 ：对市面上的 MySQL、Oracle、PostgreSQL 等数据库有一个基本印象，知道它们分别用在什么地方。
4. 学会安装和配置 MySQL ：掌握在 Linux 上安装 MySQL 的完整步骤，并学会如何配置使其能被远程访问。
5. 掌握基本的用户和权限管理 ：学会在 MySQL 中创建一个可以远程登录的新用户。
6. 学会创建数据库和表 ：掌握创建自己的数据库和数据表的基本命令，并理解字符集的作用。
7. 掌握最核心的数据操作 ：学会如何向表中插入数据（ INSERT ）以及如何查询数据（ SELECT ）。
8. 学会使用 Navicat 工具 ：掌握通过 Navicat 图形化工具连接数据库，让操作更简单直观。
9. 通过练习巩固知识 ：动手完成所有操作，确保真正学会并记住。

...
- **常见数据类型表格**：
    ...
    | 数据类型          | 作用与特点                              | 长度范围/示例                | 使用场景                     |
    |-------------------|----------------------------------------|-----------------------------|------------------------------|
    | INT               | 存储整数                               | -2^31 到 2^31-1            | 编号、年龄、数量             |
    | BIGINT            | 存储大整数                             | -2^63 到 2^63-1            | 大数据量编号                 |
    | SMALLINT          | 存储小整数                             | -32768 到 32767            | 小范围数值                   |
    | TINYINT           | 存储极小整数                           | -128 到 127                 | 状态值、标志位               |
    | DECIMAL(M,D)      | 存储精确小数，M 总位数，D 小数位       | DECIMAL(10,2)               | 金额、价格                   |
    | FLOAT             | 存储单精度浮点数，精度有限             | FLOAT                       | 近似小数，科学计算           |
    | DOUBLE            | 存储双精度浮点数，精度更高             | DOUBLE                      | 高精度小数                   |
    | VARCHAR(n)        | 存储可变长度文本，n 是最大字符数       | VARCHAR(50)                 | 姓名、地址                   |
    | CHAR(n)           | 存储固定长度文本，n 是字符数           | CHAR(4)                     | 固定长度编号、代码           |
    | TEXT              | 存储大文本，无固定长度限制             | TEXT                        | 文章、备注                   |
    | DATE              | 存储日期（年月日）                     | DATE                        | 出生日期、录入日期           |
    | TIME              | 存储时间（时分秒）                     | TIME                        | 一天内时间                   |
    | DATETIME          | 存储日期和时间                         | DATETIME                    | 具体时间点                   |
    | TIMESTAMP         | 存储日期和时间，自动更新               | TIMESTAMP                   | 记录更新时间                 |
    | ENUM('值1','值2')| 存储预设值，只能选一个                 | ENUM('M','F')               | 性别、状态                   |
    | SET('值1','值2') | 存储预设值，可选多个                   | SET('A','B','C')            | 标签、兴趣                   |
